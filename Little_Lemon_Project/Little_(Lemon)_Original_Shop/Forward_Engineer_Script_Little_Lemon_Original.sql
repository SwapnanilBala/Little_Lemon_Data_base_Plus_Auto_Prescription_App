...
CREATE TABLE IF NOT EXISTS `little_lemon_db`.`offline_orders` (
  ...
  INDEX `fk_menu_no` (`menu_no` ASC) VISIBLE,
  CONSTRAINT `fk_menu_no`
    FOREIGN KEY (`menu_no`)
...
-- -----------------------------------------------------
-- Placeholder table for view `little_lemon_db`.`order_details`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `little_lemon_db`.`order_details` (`name` INT, `id` INT, `quantity` INT, `time_` INT, `table_number` INT);

-- -----------------------------------------------------
-- Placeholder table for view `little_lemon_db`.`table_status`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `little_lemon_db`.`table_status` (`TABLE_NO` INT, `PRESENT_STATUS` INT, `TIME_BOOKED` INT);
...
-- -----------------------------------------------------
-- procedure offline_payments_insertions
-- -----------------------------------------------------

DELIMITER $$
USE `little_lemon_db`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `offline_payments_insertions`(	IN customer_id_offline INT,
                                            	IN payment_method VARCHAR(25),
                                            	IN comments VARCHAR(35)
                                            )
BEGIN
		DECLARE total_bill FLOAT;
    SELECT 
    			SUM(menus.price * offline_orders.quantity) INTO total_bill
    FROM menus 
    JOIN offline_orders ON offline_orders.menu_no = menus.menu_id
    WHERE offline_orders.offline_customers_id = customer_id_offline;
    
    INSERT INTO offline_payments(offline_customers_id,total_bill,payment_type,remarks)
    VALUES (customer_id_offline, total_bill, payment_method, comments);
END$$
...
BEGIN
    ...
    VALUES (new_customer_id, new_menu_no, new_quantity);
END$$
...
-- -----------------------------------------------------
-- procedure payment_insertion
-- -----------------------------------------------------

DELIMITER $$
USE `little_lemon_db`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `payment_insertion`(
  IN order_no_in int,
  IN table_no_in int,
	IN final_bill_in float,
	IN payment_type_in varchar(25),
  IN remarks_in varchar(55)
)
BEGIN
		INSERT INTO payments(order_no, table_no, final_bill, payment_type, remarks)
    VALUES (order_no_in, table_no_in, final_bill_in, payment_type_in, remarks_in);
END$$
...
-- -----------------------------------------------------
-- procedure total_bill
-- -----------------------------------------------------

DELIMITER $$
USE `little_lemon_db`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `total_bill`(IN customer_id int)
BEGIN 
		DECLARE total_bill float;

		SELECT SUM(menus.price * orders.quantity)
    into total_bill
    FROM menus
    JOIN orders on orders.menu_no = menus.menu_id
    WHERE orders.customer_id = customer_id;
    
    SELECT total_bill AS Bill;
END$$
...
-- -----------------------------------------------------
-- View `little_lemon_db`.`order_details`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `little_lemon_db`.`order_details`;
USE `little_lemon_db`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `little_lemon_db`.`order_details` AS select `little_lemon_db`.`customers`.`customer_name` AS `name`,`little_lemon_db`.`orders`.`order_id` AS `id`,`little_lemon_db`.`orders`.`quantity` AS `quantity`,`little_lemon_db`.`bookings`.`booking_slot` AS `time_`,`little_lemon_db`.`tables_`.`table_number` AS `table_number` from (((`little_lemon_db`.`customers` join `little_lemon_db`.`orders` on((`little_lemon_db`.`orders`.`customer_id` = `little_lemon_db`.`customers`.`customer_id`))) join `little_lemon_db`.`bookings` on((`little_lemon_db`.`bookings`.`booking_id` = `little_lemon_db`.`customers`.`booking_no`))) join `little_lemon_db`.`tables_` on((`little_lemon_db`.`bookings`.`table_no` = `little_lemon_db`.`tables_`.`table_number`)));
...
USE `little_lemon_db`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `little_lemon_db`.`table_status` AS select `little_lemon_db`.`tables_`.`table_number` AS `TABLE_NO`,`little_lemon_db`.`tables_`.`availability` AS `PRESENT_STATUS`,`little_lemon_db`.`tables_`.`booking_time` AS `TIME_BOOKED` from (`little_lemon_db`.`tables_` join `little_lemon_db`.`bookings` on((`little_lemon_db`.`bookings`.`table_no` = `little_lemon_db`.`tables_`.`table_number`)));
USE `little_lemon_db`;
