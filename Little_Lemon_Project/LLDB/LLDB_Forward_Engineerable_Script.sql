...
-- -----------------------------------------------------
-- Table `lldb`.`booking_log`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `lldb`.`booking_log` (
  `booking_log_id` INT NOT NULL AUTO_INCREMENT,
  `details` VARCHAR(20) NULL DEFAULT NULL,
  `type` VARCHAR(15) NULL DEFAULT NULL,
  `alter_time` DATETIME NULL DEFAULT NULL,
...
TRIGGER `lldb`.`booking_delete`
AFTER DELETE ON `lldb`.`bookings`
FOR EACH ROW
BEGIN
		INSERT INTO Booking_Log(details,type,alter_time) 
    VALUES (OLD.booking_type,"DELETION", OLD.Booking_slot);
END$$

USE `lldb`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `lldb`.`booking_insert`
AFTER INSERT ON `lldb`.`bookings`
FOR EACH ROW
BEGIN
		INSERT INTO Booking_Log(details,type,alter_time) 
    VALUES (NEW.booking_type,"INSERTION", NEW.Booking_slot);
END$$

USE `lldb`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `lldb`.`booking_update`
AFTER UPDATE ON `lldb`.`bookings`
FOR EACH ROW
BEGIN
		INSERT INTO Booking_Log(details,type,alter_time) 
    VALUES (NEW.booking_type,"UPDATION", NEW.Booking_slot);
END$$
